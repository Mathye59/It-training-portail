...
// (facultatif) ton fetch est OK, mais il référence setFormations ici non défini
...
const dummyFormations: Formation[] = Array.from({ length: 40 }, (_, i) => ({
  ...
  image: '/images/formation-numerique-propriete-intellectuelle.jpg', // public/ -> /images/...
...
const TrainingFormations: React.FC = () => {
  ...
  const currentFormations = dummyFormations.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(dummyFormations.length / ITEMS_PER_PAGE);

  return (
    <div className="space-y-6 text-blueDarkIT dark:text-white">
      {currentFormations.map((formation) => (
        ...
        >
          ...
          <div className="absolute flex flex-col items-end md:flex-row top-2 right-1 gap-1 text-xs text-white text-right">
            ...
            <span
              className={`px-2 py-1 rounded-md w-fit ${
                formation.etatSession === 'Formation en cours' ? 'bg-turquoise3' : 'bg-finlandais'
              }`}
            >
              {formation.etatSession === 'Formation en cours' ? 'Session en cours' : 'Session clôturée'}
            </span>
          </div>
          ...
          <div className="flex flex-col justify-between p-4 leading-normal text-left w-[80%]">
            ...
            <p className="mb-1 text-sm text-penn dark:text-white/90">{formation.description}</p>
            <span className="text-xs text-yale dark:text-white/70">{formation.niveau}</span>
          </div>
        </Link>
      ))}
